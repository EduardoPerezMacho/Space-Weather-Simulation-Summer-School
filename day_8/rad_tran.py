...
if __name__ == '__main__':  # main code block
    ...
    tau_o = calc_tau_all(SZA, density_o, h_o, euv_info['ocross'])
    tau_o2 = calc_tau_all(SZA, density_o2, h_o2, euv_info['o2cross'])
    tau_n2 = calc_tau_all(SZA, density_n2, h_n2, euv_info['n2cross'])
    # Need to calculate tau for N2 and O2, and add together...
    # and do this for all of the wavelengths...
    tau = tau_o + tau_o2 + tau_n2 # add together
    ...
    plot_value_vs_alt(alts, tau[5], 'tau_all.png', 'tau ()')

    Qeuv_o = calculate_Qeuv_all(density_o,
                            intensity_at_inf,
                            tau,
                            euv_info['ocross'],
                            energies,
                            efficiency)
    Qeuv_o2 = calculate_Qeuv_all(density_o2,
                            intensity_at_inf,
                            tau,
                            euv_info['o2cross'],
                            energies,
                            efficiency)
    Qeuv_n2 = calculate_Qeuv_all(density_n2,
                            intensity_at_inf,
                            tau,
                            euv_info['n2cross'],
                            energies,
    ...
    Qeuv = Qeuv_o + Qeuv_o2 + Qeuv_n2 # add together

    # plot out to a file:
    plot_value_vs_alt(alts, Qeuv_o, 'o_qeuv.png', 'Qeuv - O (W/m3)')
    plot_value_vs_alt(alts, Qeuv_o2, 'o2_qeuv.png', 'Qeuv - O (W/m3)')
    plot_value_vs_alt(alts, Qeuv_n2, 'n2_qeuv.png', 'Qeuv - O (W/m3)')
    plot_value_vs_alt(alts, Qeuv, 'all_qeuv.png', 'Qeuv - O (W/m3)')
